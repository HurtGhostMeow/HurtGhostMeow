...
/* Only phosphor pixels animate. The enclosure stays still. */
.phosphor{animation:ignite 9s linear infinite both;animation-delay:var(--phase,0s)}
@keyframes ignite{0%,3%{opacity:.06}4%{opacity:1}7%{opacity:.8}65%{opacity:.8}73%{opacity:.25}80%,100%{opacity:.06}}
@media (prefers-reduced-motion:reduce){.phosphor{animation:none;opacity:1}}
</style><defs><radialGradient id="bg"><stop stop-color="#30212e"/><stop offset="1" stop-color="#100e16"/></radialGradient><filter id="glow" x="-100%" y="-100%" width="300%" height="300%"><feGaussianBlur stdDeviation="2.2"/></filter><pattern id="scan" width="4" height="4" patternUnits="userSpaceOnUse"><path d="M0 3.5H4" stroke="#000" stroke-opacity=".16"/></pattern></defs><rect x="1" y="1" width="958" height="${h-2}" rx="24" fill="url(#bg)" stroke="#694153"/><g font-family="Consolas,monospace" fill="#eec5d6">${body}</g><rect x="2" y="2" width="956" height="${h-4}" rx="23" fill="url(#scan)"/></svg>\n`;}
...
for(const c of 'HURTGHOSTMEOW'){
  ...
  glyph.forEach((row,y)=>[...row].forEach((bit,x)=>{
    const px=cursor+x*13, py=91+y*13;
    dots+=`<circle cx="${px}" cy="${py}" r="4" fill="#352733"/>`;
    if(bit==='1') dots+=`<g class="phosphor" style="--phase:${((px-64)/702*1.8).toFixed(3)}s"><circle cx="${px}" cy="${py}" r="5" fill="#efadc9" filter="url(#glow)" opacity=".65"/><circle cx="${px}" cy="${py}" r="4" fill="#efadc9"/><circle cx="${px}" cy="${py}" r="1.8" fill="#ffe0ec" opacity=".8"/></g>`;
  }));
  cursor+=(glyph[0].length+1)*13;
}
const header=svg(248,'HurtGhostMeow · Pink VFD',`<text x="60" y="39" font-size="12" letter-spacing="3">VFD / PERSONAL SIGNAL</text><circle cx="889" cy="35" r="4" fill="#efadc9"/>${dots}<text x="60" y="193" font-size="17">有朋自远方来，不亦乐乎</text><text x="60" y="220" font-size="12" fill="#bc92a7" letter-spacing="2">HARDWARE · CODE · CATS</text>`);
...
for(const day of days){
 ...
 pixels+=`<rect x="${x-4.5}" y="${y-4.5}" width="9" height="9" rx="2" fill="${colors[0]}"><title>${day.date}: ${day.count} contributions</title></rect>`;
 if(day.level) pixels+=`<g class="phosphor" style="--phase:${(Math.floor(offset/7)/52*1.8).toFixed(3)}s"><circle cx="${x}" cy="${y}" r="5.4" fill="${colors[day.level]}" filter="url(#glow)" opacity=".7"/><rect x="${x-4.5}" y="${y-4.5}" width="9" height="9" rx="2" fill="${colors[day.level]}"><title>${day.date}: ${day.count} contributions</title></rect></g>`;
}
