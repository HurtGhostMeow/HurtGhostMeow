...
const header=svg(248,'HurtGhostMeow · Pink VFD',`<text x="60" y="39" font-size="12" letter-spacing="3">VFD / PERSONAL SIGNAL</text><circle cx="889" cy="35" r="4" fill="#efadc9"/><g filter="url(#glow)" opacity=".65">${dots}</g>${dots}<text x="60" y="193" font-size="17">有朋自远方来，不亦乐乎</text><text x="60" y="220" font-size="12" fill="#bc92a7" letter-spacing="2">HARDWARE · CODE · CATS</text>`);
...
for(const day of days){
 ...
 if(day.level) pixels+=`<circle cx="${x}" cy="${y}" r="5.4" fill="${colors[day.level]}" filter="url(#glow)" opacity=".7"/>`;
